...
export const countMostUsedNodeModules = (data: NodeModuleFrequency): MostUsedNodeModules => {
  const sortedModulesByFrequency = Object.entries(data).sort((a, b) => b[1] - a[1]);
  ...
  const mostUsedNodeModules = topThreeModules
    .map(([key, value]) => [key, getPercentageString(value, sumOfModuleFunctions)])
